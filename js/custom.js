...
function create_todo_list_item(i){
    ...
    todo_item.append($(`<input type="checkbox" class="col-1">`).attr('checked', todos[i].done).change(check));

    let span = $(`<span class="col-8">${todos[i].task}</span>`);
    todo_item.append(span)
    if(todos[i].done){
        $(span).addClass('todo-done')
    }
...
function check(ev){
    ...
    todos[id].done = ev.target.checked;
    refresh_todos()
